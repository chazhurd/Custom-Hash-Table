...
public class Hashtable {
   ...
   HashtableRecord result = theList.getLast();
   Object finalresult = result.value;
   return finalresult;
    }
    
    else if(theList !=null && theList.size()>1)
    {
    	
    	int x = 0;
    	while(theList.size() > x)
    	{
    		{
    			 HashtableRecord result = theList.get(x);
